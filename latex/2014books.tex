...
% Font fallback chain for Chinese typesetting
\IfFontExistsTF{Noto Serif CJK SC}{%
  \setromanfont[Mapping=tex-text,BoldFont=Noto Sans CJK SC]{Noto Serif CJK SC}%
}{%
  \IfFontExistsTF{AR PL SungtiL GB}{%
    \setromanfont[Mapping=tex-text,BoldFont=WenQuanYi Micro Hei]{AR PL SungtiL GB}%
  }{%
    \setromanfont[Mapping=tex-text,BoldFont=WenQuanYi Micro Hei]{WenQuanYi Zen Hei}%
  }%
}
...
\begin{document}
% Cover: use img/cover.pdf if available, otherwise generate text cover
\IfFileExists{img/cover.pdf}{%
  \includegraphics[scale=0.8]{img/cover.pdf}%
}{%
  \thispagestyle{empty}%
  \vspace*{\fill}%
  \begin{center}%
    {\Huge\bfseries\color{colorchapter} 2014年值得你阅读的100本书}\\[2em]%
    {\Large Conan Xin}\\[1em]%
    {\large Markdown Book Project}\\[3em]%
    {\normalsize 整理自第一财经周刊专题}%
  \end{center}%
  \vspace*{\fill}%
  \newpage%
}
